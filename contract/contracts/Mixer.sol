...
contract Mixer is Initializable, ContextUpgradeable, OwnableUpgradeable {
    ...
    function depositTokens(
        ...
        require(msg.value >= 10 ** 14, "Mixer: Fee to contract not sent!");

        if (addressDeposits[currentContract] == 3) {
            createNewInnerContract();
        }

        if (_erc20Addr == address(0)) {
            require(
                msg.value - 10 ** 14 >= 3 * 10 ** 14,
                "Mixer: Min balance to deposit not sent!"
            );
            currentContract.transfer(msg.value - 10 ** 14);
