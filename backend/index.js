...
app.get("/get/balance/:myaddress/:crypto", async (req, res) => {
  const query =
    '*[_type == "txTracker" && to == $walletAddress && coin == $coin && status == $status] {amount}';
  const params = {
    walletAddress: req.params.myaddress,
    status: "pending",
    coin: req.params.crypto,
  };
  const result = await client.fetch(query, params);
  const summary =
    result.length != 0
      ? result.map((bal) => bal.amount).reduce((acc, bal) => bal + acc)
      : 0;
...
app.get("/get/data/:myaddress/:crypto", async (req, res) => {
  const query =
    '*[_type == "txTracker" && to == $walletAddress && coin == $coin && status == $status && isCEX == $isCEX] {_id,from,contract,amount,coin}';
  const params = {
    walletAddress: req.params.myaddress,
    status: "pending",
    coin: req.params.crypto,
...
app.get("/get/contractDataResult/:mycontract", async (req, res) => {
  const query =
    '*[_type == "txTracker" && contract == $contractAddress && isCEX == $isCEX && status == $status] {_id,from,to,contract,amount,coin,tokenAddress}';
  const params = {
    contractAddress: req.params.mycontract,
    status: "pending",
    isCEX: true,
  };
  const result = await client.fetch(query, params);
...
app.get("/get/contractCompliance/:mycontract", async (req, res) => {
  const query =
    '*[_type == "txTracker" && contract == $contractAddress && status == $status] {_id,from,to,contract,amount,coin,tokenAddress}';
      ...
      const receipt = await provider
        ...
        .then(async () => {
          await client
            ...
            .set({ status: "paid" })
            .commit()
            .then(console.log("paid", e.to)) // paid to Owner
            // res.send("Withdrawal successfully completed.")
            .catch((e) => `Error is: ${e}`);
        })
        .catch((e) => {
...
app.post("/save/trnx/", async (req, res) => {
  const { from, to, coin, tokenAddress, contract, amount, status, isCEX } =
    req.body;
